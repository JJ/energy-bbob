...
This is why our methodology needs an additional step to register the actual energy consumed by the specific workload we are interested in. In order to do that, in this paper we will follow the same methodology as in other papers such as \cite{wivace23}: measure a common function as baseline; averages obtained here will be subtracted from the time and energy consumed by the functions we want to study later.
...
<<evostar.bbob.setup, echo=F, message=F, fig.height=4, fig.cap="Time in seconds for every experiment generating 40K chromosome vectors with 128,256,512 dimensions, using float or double for every one of them">>=
library(ggplot2)
library(ggthemes)
library(dplyr)
...
<<evostar.bbob.fixed, echo=F, message=F, fig.height=4, fig.cap="Time in seconds for every experiment generating 40K array or vector chromosomes with 128 elements, using float or double for every one of them">>=
...
We are, however, also interested in the actual type we use to represent this number vector if it has any influence in the energy it consumes. C++ includes some fixed-size types, such as {\sf array}. From the point of view of programming, this needs the array size to be constant, which means that you need to generate and compile different sources for every size; this is why we have run an experiment just for size = 128. This is shown in Figure \ref{fig:evostar.bbob.fixed}; colors again represent the type used for every number, while shape varies with the kind of data structures, fixed-sized array as dots, and variable-sized vector as triangles. There is a clear difference between the two, in running time as well as energy consumed, with the former consuming almost one eighth of the joules and taking almost half of the time. Again, energy consumed by single and double precision is almost the same, differing only in the time it takes. Since the only operation we are applying here is random number generation, this would mean that generating random doubles consumes the same amount of energy as generating single-precision numbers\footnote{Since the RNG used is for 64 bits, it is very likely that what it is actually doing is {\em always} generating that amount of bits and then picking 32 for single-precision {\sf floats}.}.

We should emphasize that this is the baseline measurement. We will measure the energy consumed by a selected sample of BBOB functions next.
...
<<evostar.bbob.compares, echo=F, message=F, warning=F, fig.height=5, fig.show="hold", fig.cap="PKG energy consumed in every experiment evaluating 40K chromosomes with 128 dimensions, using float or double for every one of them. Please note the $y$ axes have different scales">>=
...
But we are more interested in comparing it with the variable-size {\sf vector} data structure, which we do in Figure \ref{fig:evostar.bbob.compares}, for single precision on the left-hand side, double on the right. It is hard to see how the fixed size data structure could be better for {\sf float}, since the measurement is 0 for the variable-size data structure, the {\sf vector}, in some cases. But even in cases where it is not 0 it is almost never better. A small (but significant) advantage for the more heavyweight functions, that is all. But let us remember that {\sf double} arrays were better than these and let us compare the right-hand side panel. Again, in most cases (except for {\sf katsuura}), {\sf array}s are better, consuming less energy, or almost the same. So, this adds to our previous preliminary conclusion that double precision is better by saying that double precision {\sf vector}s are in most cases the data structure that consumes the least.
...
With the current methodology, we have at least been able to ascertain than changing representation of individual values from double to float will not have a big impact, or at least one that can offset the disadvantage of several digits of precision; in some cases, it can even result in worse carbon footprint, since most floating-point operations in low level languages, at least from what we can measure in C++, are optimized for {\sf double}. Using fixed-size data structures, on the other hand, gives us a mixed bag of results. It will certainly lead to no advantage in the fitness function itself; but any evolutionary algorithm also needs to allocate memory, and in operations that include it, like creating the chromosome, fixed-size data structures like C++'s {\sf array} are much faster and also consume less energy, as shown in Figure \ref{fig:evostar.bbob.fixed}, almost 1/8th of the energy, and that includes overhead by the system at large.

But this fact, along with the problems with measuring precisely energy consumed by the functions we are interested in, inspires our lines of future work. Since what we are using as baseline, creation of chromosomes, it is itself a factor that needs measurement since different data structures will have very different carbon footprint, we need to redesign the set of experiments trying to find a way of eliminating system overhead that does not include this phase of creation; at the same time, those baseline measurements need to be lightweight enough so that the actual functions we are interested in take at least as much time, and consume as much energy than the baseline itself, or, hopefully, more. We have had such effective measurements only in the cases of the functions that consume more energy like {\sf katsuura} or {\sf rastrigin}.

Finally, developing a methodology for measuring is interesting, but the objective we have ahead is to find a way to effectively optimize energy consumption for floating-point fitness functions. Low-level languages offer many ways to do it, sometimes descending to even lower levels using intrinsics \cite{latkin2014twofolds} or even low-precision representations \cite{tagliavini2017transprecision} in a context where they can actually have an impact reducing the amount of energy consumed.

\section*{Acknowledgements and data availability}

This work is supported by the Ministerio espa\~{n}ol de Econom\'{\i}a y
Competitividad (Spanish Ministry of Competitivity and Economy) under project PID2023-147409NB-C22. Source and data available from \url{https://github.com/JJ/energy-bbob} under a GPL license.
