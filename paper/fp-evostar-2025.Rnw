...
In C++, in particular, several strategies and tools concerning floating-point operations have been developed to promote green computing. Using {\em transprecision computing} techniques Tagliavini et al. \cite{tagliavini2017transprecision} propose an extended FP type system with hardware support, to implement transprecision computing on ultra low-power (ULP) embedded platforms. They propose binary8 with an 8-bit format and low precision using a 3-bit mantissa and binary16alt, a 16-bit formant with a higher dynamic range using an
% Please explain this better
8-bit exponent. The authors developed a C++ library to evaluate their proposal and run experiments in a dedicated transprecision FP unit. Experimental results achieved a reduction in energy consumption of up to 30\%. These experiments also show that 90\% of FP operations can scaled down to 8-bit or 16-bit formats. As a result of tuning the precision and vectorization, they decreased on average the execution time by 12\% and memory accesses by 27\%.
