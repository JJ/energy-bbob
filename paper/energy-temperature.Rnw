...
ggplot(bbob.variable[ bbob.variable$work == "katsuura",], aes(x=cumulative_seconds, y=watts,shape=size)) +
  geom_point( aes(color=type)) + scale_color_brewer(palette="Set1") +
  labs(title="Katsuura", y="Power (watts)", x="Cumulative seconds")+theme(legend.position="none")


ggplot(bbob.variable[ bbob.variable$work == "schaffers",], aes(x=cumulative_seconds, y=watts,shape=size, group=type,color=type)) +
  geom_point( aes(color=type)) + scale_color_brewer(palette="Set1") +
  labs(title="Schaffer's", y="Power (watts)", x="Cumulative seconds")+theme(legend.position="none")

ggplot(bbob.variable[ bbob.variable$work == "bent_cigar",], aes(x=cumulative_seconds, y=watts,shape=size, group=type,color=type)) +
  geom_point( aes(color=type)) + scale_color_brewer(palette="Set1") +
  labs(title="Bent cigar", y="Power (watts)", x="Cumulative seconds")+theme(legend.position="none")
...
The picture drawn in Figure \ref{fig:europar25.power} is quite different; for starters, power seems to cluster around 60 watts independently of the function and other parameters for the variable-size data, 56 for fixed size.
